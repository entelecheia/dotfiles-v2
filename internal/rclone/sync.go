...
// retries controls the --retries value (e.g., 3 for pull, 1 for push where
// permission errors are permanent).
func copyArgs(cfg *Config, paths *Paths, retries int) []string {
	args := []string{
...
func Pull(ctx context.Context, runner *exec.Runner, cfg *Config, paths *Paths, dryRun bool) error {
	...
	args := append([]string{"copy", cfg.RemotePath, cfg.LocalPath}, copyArgs(cfg, paths, 3)...)
	if dryRun {
...
func Push(ctx context.Context, runner *exec.Runner, cfg *Config, paths *Paths, dryRun bool) error {
	...
	args := append([]string{"copy", cfg.LocalPath, cfg.RemotePath}, copyArgs(cfg, paths, 1)...)
	if dryRun {
		args = append(args, "--dry-run")
	}
...
const skipFileHeader = "# Auto-generated by dot clone — files skipped due to sync errors\n# Clear with: dot clone skip clear\n"
