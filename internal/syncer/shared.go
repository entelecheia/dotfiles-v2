...
// MaterializeRuntimeExcludesFile writes the shared-folder exclude list to a
// per-run file under configDir. Git-tracked relpaths are no longer written
// here — tracked files are sync payload (see MaterializeTrackedIncludesFile).
func MaterializeRuntimeExcludesFile(configDir string, entries []SharedEntry) (string, error) {
	...
	b.WriteString("# Runtime excludes: shared folders.\n")
	for _, e := range entries {
