...
func printUsecases(cmd *cobra.Command) {
	...
	section(p, "7. Local rsync mirror (gdrive-sync)",
		"Push-first workspace → mirror; GDrive-origin changes stage into <workspace>/inbox/gdrive/ via intake.",
		[]usecase{
			...
			{"dotfiles gdrive-sync setup --pull-interval=15m",
				"Also deploy an intake scheduler at the given cadence (pass 0 to remove)"},
			...
			{"dotfiles gdrive-sync intake",
				"Stage GDrive-origin new/modified files into inbox/gdrive/<ts>/ (size+mtime)"},
			...
			{"dotfiles gdrive-sync status",
				"Paths, propagation policy, push/intake schedulers, last-push/intake"},
