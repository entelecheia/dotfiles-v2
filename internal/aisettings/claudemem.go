...
func (m *ClaudeMemManager) copilotWatches() []transcriptWatch {
	...
	for _, eventsPath := range eventFiles {
		...
		// Read first line to extract workspace from session.start event.
		firstLine := raw
		...
		if json.Unmarshal(firstLine, &event) != nil || event.Type != "session.start" {
			continue
		}
		workspace := event.Data.Context.GitRoot
		if workspace == "" {
			workspace = event.Data.Context.CWD
		}
...
func copilotTranscriptSchema() transcriptSchema {
	return transcriptSchema{
		...
		Events: []transcriptEvent{
			{Name: "session-start", Match: equals("type", "session.start"), Action: "session_init", Fields: map[string]any{"prompt": "data.prompt"}},
			{Name: "user-turn", Match: equals("type", "user.turn"), Action: "session_init", Fields: map[string]any{"prompt": "data.message"}},
			{Name: "assistant-turn", Match: equals("type", "assistant.turn"), Action: "assistant_message", Fields: map[string]any{"message": "data.message"}},
			{Name: "tool-call", Match: equals("type", "tool.call"), Action: "tool_use", Fields: map[string]any{"toolId": "data.toolCallId", "toolName": "data.toolName", "toolInput": "data.args"}},
			{Name: "tool-result", Match: equals("type", "tool.result"), Action: "tool_result", Fields: map[string]any{"toolId": "data.toolCallId", "toolResponse": "data.result"}},
...
func (m *ClaudeMemManager) Install(ctx context.Context) (ClaudeMemInstallResult, error) {
	...
	for _, target := range []struct {
		path  string
		style mcpEntryStyle
	}{{m.KimiMCPPath(), mcpEntryDefault}, {m.KiroMCPPath(), mcpEntryKiro}, {m.CopilotMCPPath(), mcpEntryCopilot}} {
		changed, err := ensureMCPEntry(target.path, m.DotPath, target.style)
		if err != nil {
			return ClaudeMemInstallResult{}, err
		}
		if changed {
...
type mcpEntryStyle int

const (
	mcpEntryDefault mcpEntryStyle = iota
	mcpEntryKiro
	mcpEntryCopilot
)

func ensureMCPEntry(path, dotPath string, style mcpEntryStyle) (bool, error) {
